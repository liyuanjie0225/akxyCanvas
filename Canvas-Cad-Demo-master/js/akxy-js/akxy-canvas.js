    ...
    var akxyCad = {
        ...
        drawQuake: function drawQuake(quakeTable) {
            ...
            for (var i in quakeTable) {
                ...
                var circleStyle = getQuakeSize(energy, radius, 0.6); // 换成应力的style  李元杰
                var x, y, z;
        ...
        //  地音绘制  李元杰
        drawEarthSound: function drawEarthSound(earthSoundTable) {
            var layercad = layercadInfo.layercad;
            var circleVectors = [];
            objEarthSoundTable = JSON.stringify(earthSoundTable);
            window.localStorage.setItem("EarthSoundTable", objEarthSoundTable);
            var startAngle = 0;
            var endAngle = Math.PI * 2;
            var radius = Number(pointSize.quake);
            ...
            for (var i in earthSoundTable) {
                console.log(earthSoundTable[i])
                var energy = earthSoundTable[i].Energy;
                if (earthSoundTable[i].Is < 0.3) {
                    circleStyle = {
                        fillColor: "green",
                        strokeColor: "green",
                        fill: false,
                ...
                else if (earthSoundTable[i].Is >= 0.3 && earthSoundTable[i].Is < 0.5) {
                    circleStyle = {
                        fillColor: "Yellow",
                        strokeColor: "green",
                        fill: false,
                ...
                else if (earthSoundTable[i].Is >= 0.5 && earthSoundTable[i].Is < 0.8) {
                    circleStyle = {
                        fillColor: "Orange",
                        strokeColor: "green",
                        fill: false,
                ...
                var x, y, z;
                if (layercadInfo.isTrueCoordinate) { //真实矿图坐标
                    x = earthSoundTable[i].X;
                    y = earthSoundTable[i].Y;
                    z = earthSoundTable[i].Z;
                } else { //剖面坐标
                    x = earthSoundTable[i].PX;
                    y = earthSoundTable[i].PY;
                    z = earthSoundTable[i].PZ;
                }
                //   鼠标悬停显示
                var cadEarthSoundInfo = {
                    "Type": "EarthSound",
                    "OccurrenceTime": earthSoundTable[i].OccurrenceTime,
                    "ShowEnergy": earthSoundTable[i].ShowEnergy,
                    "Z": earthSoundTable[i].Z,
                    "X": earthSoundTable[i].X,
                    "Y": earthSoundTable[i].Y
                }
                var circle = new AKXY.Circle(x - middleX, y - middleY, radius, startAngle, endAngle,false, 0, cadEarthSoundInfo,true);
                var vector = new AKXY.Vector(circle, circleStyle);
                console.log(vector)
                circleVectors.push(vector);
            };
...
(function (global, factory) {
        ...
        isShowCadInfo: function (layer, pt) {
            ...
            for (var i in cadGeometry) {
                ...
                if (dis <= radius * radius) {
                    ...
                    else if (cadInfo.Type == "EarthSound") {
                        var ox = Number(cadInfo.X);
                        var oy = Number(cadInfo.Y);
                        spanx.innerHTML = "X = " + ox.toFixed(1);
                        spany.innerHTML = "Y = " + oy.toFixed(1) + "&nbsp;&nbsp;&nbsp;&nbsp;Z=&nbsp;" + cadInfo.Z + "&nbsp;&nbsp;&nbsp;&nbsp;时间:&nbsp;" + cadInfo.OccurrenceTime + "&nbsp;&nbsp;&nbsp;&nbsp;能量大小:&nbsp;" + cadInfo.ShowEnergy;
                        console.log('1')
                    }
    ...
    */
    ...
    function Canvas(layer) {
        ...
        this.type = 'Canvas';
        this.pointsOfWarn = [];
        //绘制图形this.canvas融入了热力图中，如果用不到热力图可以自行取出  李元杰
